...
def _style_ax(ax, index, title):
    ...
    ax.text(0.0, 1.20, f"{index:02d}", transform=ax.transAxes,
            fontsize=8, fontweight="700", color=_SILVER,
            ha="left", va="bottom")
    ax.set_title(f"{title}", loc="left", fontsize=12.5,
                 fontweight="500", color=_INK, pad=10)
...
def _pill(ax, x, y, text, dx=8, dy=8, ha="left", va="center",
          color=_STEEL, weight="500"):
    """Draw a small rounded label badge anchored to a data point."""
    ax.annotate(
        text, (x, y), textcoords="offset points", xytext=(dx, dy),
        fontsize=7.5, color=color, ha=ha, va=va, fontweight=weight,
        zorder=6,
...
ax.plot([p1.x, p2.x], [p1.y, p2.y], color=_SILVER,
        linewidth=1.0, linestyle=(0, (3, 3)), zorder=2)
for p, label in [(p1, "keyword"), (p2, "list"), (p3, "tuple"), (p4, "numpy")]:
    ax.scatter(p.x, p.y, c=_ACCENT, s=46, zorder=4,
               edgecolors="white", linewidths=0.9)
    _pill(ax, p.x, p.y, label, dx=9, dy=9, ha="left")
_pill(ax, (p1.x + p2.x) / 2, (p1.y + p2.y) / 2,
      f"d = {p1.distance_to(p2):.2f}", dx=10, dy=-12, color=_CHARCOAL)
ax.margins(0.18)
...
ax.plot([seg.start.x, seg.end.x], [seg.start.y, seg.end.y],
        color=_INK, linewidth=2.0, zorder=3, solid_capstyle="round")
ax.scatter([seg.start.x, seg.end.x], [seg.start.y, seg.end.y],
           c=_ACCENT, s=42, zorder=5, edgecolors="white", linewidths=0.9)
mid = seg.midpoint
ax.scatter(mid.x, mid.y, c="white", s=46, zorder=5, marker="o",
           edgecolors=_INK, linewidths=1.4)
_pill(ax, mid.x, mid.y, "midpoint", dx=11, dy=10, color=_CHARCOAL)
_pill(ax, (seg.start.x + seg.end.x) / 2, (seg.start.y + seg.end.y) / 2,
      f"len = {seg.length:.1f}", dx=12, dy=-14, color=_STEEL)
ax.margins(0.18)
...
ax.set_aspect("equal")
ax.add_patch(plt.Circle((c1.center.x, c1.center.y), c1.radius,
                        facecolor=_FILL, edgecolor=_CHARCOAL,
                        linewidth=1.6, zorder=2))
# radius indicator
edge = Point(c1.center.x + c1.radius * np.cos(np.pi / 5),
             c1.center.y + c1.radius * np.sin(np.pi / 5))
ax.plot([c1.center.x, edge.x], [c1.center.y, edge.y],
        color=_SILVER, linewidth=1.1, linestyle=(0, (3, 3)), zorder=3)
ax.plot(c1.center.x, c1.center.y, "+", color=_CHARCOAL,
        markersize=9, markeredgewidth=1.6, zorder=4)
pin = Point(3, 4)
ax.scatter(pin.x, pin.y, c=_ACCENT, s=46, zorder=5,
           edgecolors="white", linewidths=0.9)
_pill(ax, pin.x, pin.y, "contains", dx=9, dy=9, color=_CHARCOAL)
_pill(ax, (c1.center.x + edge.x) / 2, (c1.center.y + edge.y) / 2,
      f"r = {c1.radius:.0f}", dx=6, dy=8, color=_STEEL)
...
_pill(ax, cx, cy, f"area = {triangle.area:.1f}", dx=0, dy=0,
      ha="center", color=_CHARCOAL)
ax.margins(0.18)
...
ax.add_patch(plt.Circle((circ2.center.x, circ2.center.y), circ2.radius,
                        facecolor="none", edgecolor=_SILVER,
                        linewidth=1.3, linestyle=(0, (4, 4)), zorder=1))
ax.fill(hxs, hys, facecolor=_FILL, edgecolor=_CHARCOAL,
        linewidth=1.6, zorder=2)
all_pts = np.array(hull_points)
ax.scatter(all_pts[:, 0], all_pts[:, 1], c=_STEEL, s=26, zorder=3,
           edgecolors="white", linewidths=0.7)
ax.scatter([v[0] for v in hv], [v[1] for v in hv],
           c=_ACCENT, s=42, zorder=5, edgecolors="white", linewidths=0.9)
ax.plot(circ2.center.x, circ2.center.y, "+", color=_CHARCOAL,
        markersize=9, markeredgewidth=1.6, zorder=4)
_pill(ax, circ2.center.x, circ2.center.y + circ2.radius,
      "min circle", dx=0, dy=10, ha="center", color=_STEEL)
...
# Header --------------------------------------------------------------------
fig.text(0.5, 0.975, "Geometric Primitives", ha="center", va="top",
         fontsize=17, fontweight="600", color=_INK)
fig.text(0.5, 0.938, "Composable building blocks of the compute-geometry library",
         ha="center", va="top", fontsize=9.5, color=_STEEL)

plt.tight_layout(rect=[0.015, 0.02, 0.985, 0.90], h_pad=3.2, w_pad=2.6)
fig.savefig("public/elements.png", dpi=220, facecolor=_BG,
