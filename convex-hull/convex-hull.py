...
if __name__ == '__main__':
    ...
    for i in range(2):
        ...
        with open(f'convex-hull/fecho{i+1}.txt', 'w') as f:
            for item in P.get_indexes():
                f.write("%s\n" % item)
    plt.savefig('convex-hull/nuvens.pdf')
    plt.show()
    ...
    for k in range(0,6):
        ...
        P.plot_convex_hull(f'Test {k+1}')
        plt.tight_layout()
    plt.savefig('convex-hull/random_test.png')
    plt.show()
    ...
    for k in range(0,6):
        ...
        ax = fig.add_subplot(2,3,k+1)
        ax.plot(sizes, runtime, color = hull_color)
        prop = (np.array(h)/np.array(runtime)).mean()
        ax.plot(sizes,  [x/prop for x in h], color = points_color)
        ax.set_title(f'Test {k+1}')
        ax.set_xlabel('Tamanho da Entrada')
        ax.set_ylabel('tempo (s)')
        plt.legend(["JarvisMarchGift", "n*h"])
        plt.tight_layout()
    plt.savefig('convex-hull/runtime_test.pdf')
    plt.show()
