...
def plot_min_circle_random(mc_obj, sizes, path=None, show=False):
    ...
    for num_points in sizes:
        ...
        print(
            f"Heuristic  Center/Radius: {min_circle_heuristic[0]} / {min_circle_heuristic[1]}"
        )
        ...
        if path is not None:
            fig.savefig(
                path + f"plot_min_circle_{num_points}_points.pdf", bbox_inches="tight"
            )
        if show:
            plt.show()
...
def plot_delaunay(dt_obj, title="Delaunay Triangulation", show_circumcircles=False):
    ...
    for tri in triangles:
        ...
        ax.fill(
            xs, ys, facecolor=(*_hex_to_rgb(_MIST), 0.05), edgecolor="none", zorder=1
        )
